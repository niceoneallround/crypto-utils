...
// not all tests can run as no KMS credentials
const CODESHIP_TEST = ['test/testFormatUtils.js', 'test/testNodeCryptoUtils.js'];
...
gulp.task('codeship-test', ['pre-test', 'pp'], function () {
  return gulp.src(CODESHIP_TEST)
    .pipe(mocha({ grep: yargs.argv.grep }))
    .pipe(istanbul.writeReports())
    .pipe(istanbul.enforceThresholds({ thresholds: { statements: 80, lines: 80, functions: 80, branches: 40 } }));
});
