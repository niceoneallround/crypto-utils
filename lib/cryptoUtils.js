...
var assert = require('assert'),
    ...
    localCryptoUtils = require('./localCryptoUtils'),
    storageFormatUtils = require('./storageFormatUtils'),
    util = require('util');

// routines that utilize all convenience routines to perform the following
//
// encryptHMACConvert2StorageFormat
//  - encrypt the data
//    - use AWS.KMS to generate a key that can be use for AES-256-CBC, using the passed in master key and context
//    - encrypt the data with AES-256 using the key from AWS.KMS and a random IV
//  - create a SHA256 hmac code across the encrypted data, the iv, the encrypted key, and key context - using the AWS.KMS key
//  - create the 'encoded' representation that supports decryption and can be stored or sent over the wire, consisting of
//    - It contains the encrypted data, the iv, the encrypted key, the key context, and the hmacCode
//
// reverseStorageCheckHMACDecrypt
...
//
function encryptHMACConvert2StorageFormat(params, next) {
  'use strict';
  ...
  params.kms.generateDataKey(params.kmsKeyParams, function(err, keyInfo) {
    ...
    ciphered = localCryptoUtils.encrypt({
    ...
    // generate HMAC code
    hmacCode = localCryptoUtils.createHMAC({
    ...
    sfmt = storageFormatUtils.convert2StorageFormat({
      cipherText: ciphered,
      iv: iv,
      cipherKey: keyInfo.CiphertextBlob,
      keyContext: params.kmsKeyParams.EncryptionContext,
...
//  - Recompute the HMAC code and compare with one in the encoded representation. If different stop.
//  - Decrypt the data
function reverseStorageCheckHMACDecrypt(params, next) {
  'use strict';
  ...
  unpacked = storageFormatUtils.reverseStorageFormat(params.sfmt);
  ...
  params.kms.decryptDataKey(decryptKeyParams, function(err, keyInfo) {
    ...
    hmac = localCryptoUtils.createHMAC({
      hmacAlgorithm: 'SHA256',
      hmacKey: keyInfo.Plaintext, // use same key that was used to encrypt
    ...
    if (!localCryptoUtils.compareHMACcode(hmac.digest('base64'), unpacked.hmacCode)) {
      assert(false, util.format('Encrypted data was tampered with')); // FIXME add an exception here
      return next(err, null);
    }

    // all ok so lets decrypt data
    plain = localCryptoUtils.decrypt({
        algorithm: 'AES-256-CBC',
        cipherText: unpacked.cipherText,
        key: keyInfo.Plaintext,
        iv: unpacked.iv
...
module.exports = {
  encryptHMACConvert2StorageFormat: encryptHMACConvert2StorageFormat,
  reverseStorageCheckHMACDecrypt: reverseStorageCheckHMACDecrypt
};
