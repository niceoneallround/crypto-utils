...
//
// Command line tool that reads data from a file, then encode et al it, and finally displaying encoded format to stdout
//
commander
  .version('1.0.0')
  ...
  .option('-v, --verbose', 'display progress messages')
  .option('-i, --in <file>', 'file containing the data to encode')
...
kms = cryptoUtils.createKMSConnection({region: commander.aws_region});
...
cryptoUtils.encryptHMACEncode(
  ...
  function(err, result) {
    ...
    console.log('start:%s:end',result);
});
